...
async def get_quiz_from_id(quiz_id: str, user: User = Depends(get_actor_user)):
    try:
...
@router.get("/list")
async def get_quiz_list(user: User = Depends(get_actor_user), page_size: int | None = 10, page: int | None = 1):
    try:
        return (
...
async def import_quiz_route(quiz_id: str, user: User = Depends(get_actor_user)):
...
async def delete_quiz(quiz_id: str, user: User = Depends(get_actor_user)):
    try:
        quiz_id = uuid.UUID(quiz_id)
    except ValueError:
        raise HTTPException(status_code=400, detail="badly formed quiz id")
...
async def import_from_excel(file: UploadFile = File(), user: User = Depends(get_actor_user)) -> Quiz:
