...
async def init_editor(edit: bool, quiz_id: Optional[UUID] = None, user: User = Depends(get_actor_user)):
...
async def finish_edit(edit_id: str, quiz_input: QuizInput):
    ...
    images_to_delete = []
    old_quiz_data: Quiz | None = await Quiz.objects.get_or_none(
        id=session_data.quiz_id, user_id=session_data.user_id
    )
    ...
    if session_data.edit:
        if old_quiz_data is None:
            raise HTTPException(status_code=404, detail="Quiz not found for this edit session")
