...
@router.get("/{quiz_id}")
async def export_quiz(quiz_id: uuid.UUID, _: User = Depends(get_actor_user)):
    try:
...
@router.post("/")
async def import_quiz(file: UploadFile = File(), user: User = Depends(get_actor_user)):
...
async def export_quiz_as_excel(quiz_id: uuid.UUID, _: User = Depends(get_actor_user)):
    try:
        quiz: Quiz = await Quiz.objects.filter(Quiz.id == quiz_id).first()
    except ormar.exceptions.NoMatch:
        raise HTTPException(status_code=404, detail="Quiz not found")
