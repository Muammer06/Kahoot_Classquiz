...
from classquiz.auth import get_actor_user
...
@router.post("/")
async def upload_file(file: UploadFile = File(), user: User = Depends(get_actor_user)) -> PublicStorageItem:
...
@router.post("/raw")
async def upload_raw_file(request: Request, user: User = Depends(get_actor_user)) -> PublicStorageItem:
    if user.storage_used > settings.free_storage_limit:
        raise HTTPException(status_code=409, detail="Storage limit reached")
    file_id = uuid4()
...
async def get_file_info(file_id: UUID, user: User = Depends(get_actor_user)) -> PublicStorageItem:
...
async def mark_file_as_deleted(file_id: UUID, user: User = Depends(get_actor_user)):
    file_data = await StorageItem.objects.get_or_none(id=file_id, user=user, deleted_at=None)
    if file_data is None:
        raise HTTPException(status_code=404, detail="File not found")
...
async def update_image_data(
    file_id: UUID, data: UpdateStorageItem, user: User = Depends(get_actor_user)
) -> PublicStorageItem:
    file_data = await StorageItem.objects.get_or_none(id=file_id, user=user, deleted_at=None)
    if file_data is None:
        raise HTTPException(status_code=404, detail="File not found")
...
async def list_images(
    since: datetime | None = None, user: User = Depends(get_actor_user)
...
async def get_latest_images(count: int = 50, user: User = Depends(get_actor_user)) -> list[PrivateStorageItem]:
    count = min(count, 50)
    items = (
        await StorageItem.objects.filter(user=user)
        .limit(count)
...
@router.get("/limit")
async def get_storage_limit(user: User = Depends(get_actor_user)) -> ReturnGetStorageLimit:
