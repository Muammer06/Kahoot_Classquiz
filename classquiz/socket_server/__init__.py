...
def calculate_score(z: float, t: int) -> int:
    """Kahoot-style points: quicker correct answers score higher (0–1000). z,t in milliseconds."""
    if t <= 0:
        return 0
    t = float(t * 1000)
    safe_z = max(0.0, float(z))
    res = max(0.0, min(1.0, (t - safe_z) / t))
    return min(1000, int(res * 1000))
...
async def submit_answer(sid: str, data: dict):
    ...
    latency = float(session.get("ping", 0) or 0)
    redis_time_raw = await redis.get(f"game:{session['game_pin']}:current_time")
    if redis_time_raw is None:
        await sio.emit("error", room=sid)
        return
    time_q_started = datetime.fromisoformat(redis_time_raw)
    diff_ms = (time_q_started - now).total_seconds() * 1000  # normally ≤ 0 → abs = elapsed ms
    score = 0
    if answer_right:
        elapsed_ms = max(0.0, abs(diff_ms) - latency)
        score = calculate_score(elapsed_ms, int(float(game_data.questions[question_index].time)))
    await redis.hincrby(f"game_session:{session['game_pin']}:player_scores", session["username"], score)
    answer_data = AnswerData(
        username=session["username"],
        answer=answer,
        right=answer_right,
        time_taken=max(0.0, abs(diff_ms) - latency),
        score=score,
    )
...
async def echo_time_sync(sid: str, data: str):
    ...
    delta = now - then
    session = await get_session(sid, sio)
    # Round-trip in ms → ~one-way latency (avoid timedelta.microseconds: only fractional part!)
    rt_ms = max(0.0, delta.total_seconds() * 1000.0)
    session["ping"] = min(rt_ms / 2.0, 4000.0)
    await save_session(sid, sio, session)
