...
async def get_guest_user() -> User:
    user = await get_user_from_mail(settings.guest_email)
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Guest user not initialized",
        )
    return user


async def get_actor_user(user: User | None = Depends(get_current_user_optional)) -> User:
    if user is not None:
        return user
