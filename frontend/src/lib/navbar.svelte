...
<script lang="ts">
	...
	const anonymousMode = import.meta.env.VITE_ANONYMOUS_MODE !== 'false';
...
<nav
	class="w-screen px-4 lg:px-10 py-2 fixed z-30 top-0 {anonymousMode
		? 'bg-[#F5F5F5]/95 border-b border-neutral-200/90 shadow-sm backdrop-blur-md dark:bg-[#252526]/95 dark:border-neutral-600/50'
		: 'backdrop-blur-2xl bg-white/70 shadow-md'}"
>
	<!-- Desktop navbar -->
	<div class="hidden lg:flex lg:items-center lg:flex-row lg:justify-between">
		<div class="lg:flex lg:items-center lg:flex-row gap-1">
			...
			{/if}
			<a
				class="btn-nav border-2 rounded-sm {anonymousMode
					? 'border-[#C41230]/40 dark:border-neutral-600'
					: 'border-transparent'}"
				href="/play">{$t('words.play')}</a
			>
			{#if anonymousMode}
				<a class="btn-nav" href="/create">{$t('words.create')}</a>
	...
	<div class="lg:hidden">
		...
		<div class="flex items-center justify-between">
			...
			{/if}
			<a
				class="btn-nav flex border-2 rounded-sm {anonymousMode
					? 'border-[#C41230]/40 dark:border-neutral-600'
					: 'border-transparent'}"
				href="/play">{$t('words.play')}</a
			>
