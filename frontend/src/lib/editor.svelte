...
<script lang="ts">
	...
	const getEditID = async () => {
		...
		if (res.status === 200) {
			...
			alert(`Düzen oturumu başlatılamadı (HTTP ${res.status})`);
		}
	};
	...
	const saveQuiz = async (e: Event) => {
		...
		if (res.ok) {
			...
			try {
				...
				msg = j.detail ? (Array.isArray(j.detail) ? j.detail.map((d: { msg?: string }) => d.msg ?? JSON.stringify(d)).join(', ') : String(j.detail)) : msg;
			} catch {
				/* keep raw */
			}
			alert(msg.slice(0, 500) || 'Kayıt başarısız');
		}
	};
</script>
...
{:then _}
	<form onsubmit={saveQuiz}>
		<div
			class="grid grid-cols-6 w-screen {navbarVisible.visible
				? 'h-[calc(100vh-4rem)]'
				: 'h-screen'}"
		>
			<div>
