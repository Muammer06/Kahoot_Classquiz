...
<script lang="ts">
	...
	interface Props {
		...
		/** Oyun boyunca altta sabit gösterilecek PIN */
		game_pin?: string;
	}

	let { game_token, bg_color, game_state = $bindable(), game_pin = '' }: Props = $props();
...
{#if game_state.timer_res !== '0' && game_state.selected_question >= 0}
	<span
		...
		class:mt-[4.25rem]={game_state.control_visible}
		style="width: {(100 /
...
<div
	class="w-full h-full"
	class:pb-16={game_pin}
	class:pt-[8.5rem]={game_state.control_visible}
...
{#if game_pin}
	<div
		class="fixed bottom-0 left-0 right-0 z-50 flex flex-wrap items-center justify-between gap-3 border-t border-black/10 bg-[#1A1A1B] px-4 py-3 text-sm text-white shadow-[0_-8px_24px_rgba(0,0,0,0.18)]"
		role="status"
	>
		<span class="opacity-80">{$t('play_page.pin_game_code')}</span>
		<span class="select-all text-2xl font-black tracking-[0.35em] tabular-nums text-white sm:text-3xl">
			{game_pin}
		</span>
