...
<script lang="ts">
	...
	let game_mode = $state();
	let auto_connect = $state(data.auto_connect);
	let game_token = $state(data.game_token);
	...
	onMount(() => {
		socket.once('connect_error', (err: Error & { message: string }) => {
			errorMessage = `Bağlantı hatası: ${err.message ?? 'bilinmiyor'}`;
		});
		if (auto_connect && game_pin && game_token) {
			connect();
		} else if (!data.has_host_params) {
			errorMessage = $t('admin_page.missing_params');
		}
		tinykeys(window, {
			Enter: next_action,
			Space: next_action
...
>
	...
	{#if !success}
		{#if errorMessage !== ''}
			<p class="text-red-700 px-8 pt-8 text-lg">{errorMessage}</p>
		{:else if data.has_host_params}
			<div class="flex min-h-[40vh] w-full flex-col items-center justify-center gap-4 text-[#1A1A1B]">
				<div
					class="h-14 w-14 animate-spin rounded-full border-4 border-[#C41230]/20 border-t-[#C41230]"
					aria-hidden="true"
				></div>
				<p class="text-xl font-semibold">Oyun bekleniyor…</p>
				<p class="text-center text-sm opacity-70">PIN: <span class="select-all">{game_pin}</span></p>
			</div>
		{/if}
